...
if os.path.isdir(PATH_Dir) and os.access(PATH_Dir, os.R_OK):
   ...
   print(f'...Dir Encontrado = {os.getcwd()}')                      # ...Dir Encontrado = G:\iconos\tmp      //ruta de las imagenes .png origen para convertir.
   lstfiles = os.listdir(os.getcwd())                               # lisdir()  obtiene una lista=[]  de los nombres de todo los files. (.txt, .png, etc)
   for idx,filename in enumerate(lstfiles):
      print(f'{idx}.- {filename}')                                  # 0.- PyImageSearch_University.png       // vizualizo ruta de c/imagen .png    
